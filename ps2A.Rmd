...
```{r}
names(flightsJoined2)
flightsJoined2 %>%
  filter(is.na(arr_delay)==FALSE) %>%
  group_by(name) %>%
summarize(N = n(), avg_arr_delay = mean(arr_delay), avg_dist_km = mean(distance_km))
...
> ANSWER: The top three names with the largest ratios are Justice, Skyler, and Quinn, who have corresponding ratios of 0.97155985, 0.7727724, and 0.7625290.
