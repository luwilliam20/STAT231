...
> ANSWER: The indoor one-mile world record for IAAF women was 4:13.31, held by Genzebe Dibaba from Ethiopia. The record was set on February 17, 2016 at	Stockholm, Sweden.

```{r}
Womens_indoor <- html_table(tables[[10]]) 
Womens_indoor
```
...
> ANSWER: The fastest record is an outdoor event.

```{r}
Womens_outdoor <- html_table(tables[[8]])

Womens_outdoor1 <- Womens_outdoor %>%
  select(-Auto)%>%
  mutate(Womens_outdoor, type = "outdoor")

Womens_indoor1 <- Womens_indoor %>%
  mutate(Womens_indoor, type = "indoor")

women <- bind_rows(Womens_indoor1, Womens_outdoor1, id = NULL)
