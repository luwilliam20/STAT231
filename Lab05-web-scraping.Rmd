...
```{r}
poem <- "https://en.wikipedia.org/wiki/List_of_Emily_Dickinson_poems"
  poemtables <- poem %>%
  read_html() %>%
  html_nodes("table")
  
poemtables1 <- new <- html_table(poemtables[1])
...
```{r}
data <- data %>% mutate(title_col_mod = gsub(” “, “_”, title_col))

table_1<- table_ %>% mutate(title_col_mod = gsub(" ", "-", `First Line (often used as title)`))
table_1
